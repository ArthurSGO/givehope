    ...
    <div class="container py-4">
        <div class="d-flex flex-wrap justify-content-between align-items-start gap-3 mb-4">
            <div>
                ...
                <p class="text-muted mb-0">Descubra as próximas ações solidárias e prepare-se para contribuir.</p>
            </div>
        ...
        @if ($events->isEmpty())
            <div class="border rounded-3 p-5 text-center bg-body-tertiary">
                <h2 class="h5">Nenhum evento planejado ainda.</h2>
                <p class="text-muted mb-0">Volte em breve para acompanhar novas campanhas.</p>
            </div>
        @else
            <div class="row g-4">
                            ...
                            <div class="card-body d-flex flex-column">
                                ...
                                <ul class="list-unstyled small text-muted mb-3">
                                    ...
                                    @if ($event->highlight_needs)
                                        <li><i class="fa-solid fa-gift me-1"></i> {{ $event->highlight_needs }}</li>
                                    @endif
                                </ul>
                                <p class="small flex-grow-1">{{ Str::limit($event->description, 120) }}</p>
                                <a href="{{ route('events.show', $event) }}" class="btn btn-outline-primary mt-3">Saiba mais</a>
                            </div>
                        </div>
                    </div>
                @endforeach
