                    ...
                    <form action="{{ route('distribuicoes.store') }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="beneficiario_id" id="beneficiario_id" class="form-select" required>
                                ...
                                <option value="{{ $beneficiario->id }}" @selected(old('beneficiario_id') == $beneficiario->id)>
                                    {{ $beneficiario->nome }}
                                </option>
                                @endforeach
                            </select>
                                ...
                                <table class="table table-bordered table-striped align-middle">
                                    ...
                                    <tbody>
                                        ...
                                        <tr @class(['table-warning' => $disponivel <= 0])>
                                            ...
                                            <td class="text-center">{{ $formatQuantidade($estoque->quantidade, $estoque->unidade) }}</td>
                                            <td class="text-center">{{ $formatQuantidade($estoque->quantidade_reservada, $estoque->unidade) }}</td>
                                        ...
                                        @empty
                                        <tr>
                                            <td colspan="7" class="text-center text-muted">Nenhum item disponível para distribuição no momento.</td>
                                        </tr>
                                        @endforelse
                                    </tbody>
                                </table>
                            </div>
                            <div class="form-text">Informe apenas as quantidades que serão reservadas para esta distribuição.</div>
                        ...
                        <div class="d-flex justify-content-end gap-2">
                            ...
                            <button type="submit" class="btn btn-primary" @disabled(!$temDisponivel)>Reservar Itens</button>
                        </div>
                    </form>
                </div>
            </div>
